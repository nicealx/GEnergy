				...
				beforeSubmit: function(arr, $form, options) {
					...
					if ($form.hasClass('validate'))
					{
						...
						if ($form.find('.is-invalid').length)
						{
							...
							if ($form.find('.is-invalid:first').data('description'))
								bootbox.alert($form.find('.is-invalid:first').data('description'));
	...
	switch ($(field)[0].tagName)
	{
		case 'INPUT':
			switch ($(field).attr('type'))
			{
				case 'radio':
				case 'checkbox':
					value = $(form).find('input[name="' + $(field).attr('name') + '"]').is(':checked');
					break;
				default:
					value = $(field).val();
			}
...
function load_tt(id)
{	
	$('.list-items-b-l>li').addClass('b-r');
	$('.list-items-b-l a[data-id="'+id+'"]').parent().removeClass('b-r');

	$('#ttId li:eq(0) span:eq(1)').text(tt[id].name);
	$('#ttId li:eq(1) span:eq(1)').text(tt[id].address);
	$('#ttId li:eq(2) span:eq(1)').text(tt[id].note);
	$('#ttId li:eq(3) span:eq(1) b').text(tt[id].status);
	
	$('#ttPrize').empty().html(tt[id].prize);
	
	return false;
}

function loadParticipant(apid)
{
	loaderStart();
	$('#formTTLink').resetForm();
	...
	$.ajax({
		...
		success: function(result, textStatus, jqXHR )
		{
			...
			$('#formTTLink input[name=apid]').val(apid);
			if (data.participantid != null)
			{
				$('#formTTLink input[name=participantid]').val(data.participantid);
				$('#formTTLink input[name=tt]').val(data.ident);
				$('#formTTLink .checkBlock').removeClass('d-n');
			}
		
			$('#editData .modal-body .list-items li:eq(0) .list-item-content-modal').html(data.name);
			$('#editData .modal-body .list-items li:eq(1) .list-item-content-modal').html(data.address);
			$('#editData .modal-body .list-items li:eq(2) .list-item-content-modal').html(data.note);
			
			if (data.reason != '')
			{				
				$('#formTTLinkDangerTxt').html(data.reason).removeClass('d-none');
			}
			else
			{
				$('#formTTLinkDangerTxt').empty().addClass('d-none');
			}
